...
const Products = ({ cartId, onCartUpdate }) => {
  ...
  const createCustomProduct = async () => {
    ...
    setError('');
    try {
      // ✅ Используем productAPI вместо fetch
      const response = await productAPI.createCustom({
        ...
        imageUrl: customProduct.imageUrl || `https://placehold.co/400x300/f59e0b/fff?text=${encodeURIComponent(customProduct.name)}`
      });
      ...
      setError('Failed to create product: ' + (error.response?.data?.error || error.response?.data || error.message));
    } finally {
      setCreating(false);
    }
  };
